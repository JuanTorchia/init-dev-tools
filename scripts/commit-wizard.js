...
async function checkAndFixPermissions() {
  console.log(
    chalk.blue(
      'Checking for executable permissions in scripts and bin directories...',
    ),
  )
...
async function selectFilesToAdd() {
  ...
  if (addAll) {
    ...
    const { files } = await inquirer.prompt({
      ...
      choices: execSync('git ls-files --others --modified --exclude-standard', {
        encoding: 'utf-8',
      })
        .split('\n')
        .filter((line) => line)
...
async function commitWizard() {
  ...
  try {
    execSync(`git commit -m "[${commitType}] ${commitMessage}" --no-verify`, {
      stdio: 'inherit',
    })
